...
export function describeMdoEndpointTests(app: INestApplication, tokens: TestTokens) {
  describe('🗂️ MDO (Mapa Diário Obra) Endpoint Tests', () => {
    ...
    describe('A) Headers Controller Tests', () => {
      ...
      describe('Access Control Tests', () => {
        ...
        it('❌ user should not access header created by others (different employeeNumber)', async () => {
          // This test assumes we have a header created by admin
          const adminHeaderRes = await request(app.getHttpServer())
            .post('/mdo/headers')
            .set('Authorization', `Bearer ${adminToken}`)
            .send({
              data: new Date().toISOString().split('T')[0],
          ...
          if (adminHeaderRes.status === 201) {
            const adminHeaderBostamp = adminHeaderRes.body.bostamp;

            // Try to access with user token
            const res = await request(app.getHttpServer())
              .get(`/mdo/headers/${adminHeaderBostamp}`)
              .set('Authorization', `Bearer ${userToken}`);
    ...
    describe('B) Details Controller Tests', () => {
      describe('POST /mdo/details - Create Detail', () => {
        ...
        it('✅ should create equipment detail (codviat)', async () => {
          ...
          expect(res.status).toBe(201);
          expect(res.body.bistamp).toBe(createdHeaderBostamp);
      ...
      describe('PUT /mdo/details/:bistamp - Update Detail', () => {
        ...
        it('❌ should fail to update detail without access', async () => {
          // Create a detail with admin token
          ...
          if (adminDetailRes.status === 201) {
            const adminDetailBistamp = adminDetailRes.body.bistamp;

            // Try to update with user token (should fail)
            const res = await request(app.getHttpServer())
              .put(`/mdo/details/${adminDetailBistamp}`)
              .set('Authorization', `Bearer ${userToken}`)
              .send({
                qtt: 10
              });
    ...
    describe('C) Import From Yesterday Tests', () => {
      describe('POST /mdo/details/:bostamp/import - Import Yesterday Details', () => {
        it('✅ should import yesterday details with qtt=0 and new bistamp', async () => {
          if (yesterdayHeaderBostamp) {
            // Create a new header for today to import into
            const todayHeaderRes = await request(app.getHttpServer())
              .post('/mdo/headers')
              .set('Authorization', `Bearer ${userToken}`)
              .send({
                data: new Date().toISOString().split('T')[0],
                obra: 'IMPORT-TARGET-OBRA',
                encarregado: 'Import Target'
              });

            if (todayHeaderRes.status === 201) {
              const targetBostamp = todayHeaderRes.body.bostamp;

              // Import from yesterday
              const importRes = await request(app.getHttpServer())
                .post(`/mdo/details/${targetBostamp}/import`)
                .set('Authorization', `Bearer ${userToken}`)
                .send({
                  sourceBostamp: yesterdayHeaderBostamp
                });

              expect(importRes.status).toBe(201);
              expect(Array.isArray(importRes.body)).toBe(true);
              
              // Verify imported details
              if (importRes.body.length > 0) {
                const importedDetail = importRes.body[0];
                expect(importedDetail.qtt).toBe(0);
                expect(importedDetail.bistamp).not.toBe(importedDetail.sourceBistamp);
                expect(importedDetail.bostamp).toBe(targetBostamp);
                
                // Verify labor/equipment structure is preserved
                if (importedDetail.empresa && importedDetail.nfunc) {
                  expect(importedDetail.codviat).toBeNull();
                } else if (importedDetail.codviat) {
                  expect(importedDetail.empresa).toBeNull();
                  expect(importedDetail.nfunc).toBeNull();
                }
              }
            }
          }
        ...
        it('❌ should fail to import without permission', async () => {
          if (yesterdayHeaderBostamp) {
            // Try to import with user without permissions
            const res = await request(app.getHttpServer())
              .post(`/mdo/details/${createdHeaderBostamp}/import`)
    ...
    describe('E) Supervisor Override Tests', () => {
      describe('Supervisor Access Control', () => {
        it('✅ supervisor should update any header regardless of creation date', async () => {
          // Create header with user token
          const userHeaderRes = await request(app.getHttpServer())
            .post('/mdo/headers')
            .set('Authorization', `Bearer ${userToken}`)
            .send({
              data: new Date().toISOString().split('T')[0],
              obra: 'SUPERVISOR-TEST-OBRA',
              encarregado: 'User Encarregado'
            });

          if (userHeaderRes.status === 201) {
            const userHeaderBostamp = userHeaderRes.body.bostamp;

            // Update with admin token (supervisor)
            const updateRes = await request(app.getHttpServer())
              .put(`/mdo/headers/${userHeaderBostamp}`)
              .set('Authorization', `Bearer ${adminToken}`)
              .send({
                obra: 'SUPERVISOR-UPDATED-OBRA',
                encarregado: 'Supervisor Updated'
              });

            expect(updateRes.status).toBe(200);
            expect(updateRes.body.obra).toBe('SUPERVISOR-UPDATED-OBRA');
          }
        });
        ...
        it('✅ supervisor should delete any header regardless of creation date', async () => {
          ...
          if (userHeaderRes.status === 201) {
            ...
            // Delete with admin token (supervisor)
            const deleteRes = await request(app.getHttpServer())
              .delete(`/mdo/headers/${userHeaderBostamp}`)
              .set('Authorization', `Bearer ${adminToken}`);
    ...
    describe('F) Validation and Edge Cases', () => {
